...
public class searcher {
	...
	public static void search(String squery, String index, int offset,
			int resperpage) {
		try{
		...
		Date start = new Date();
		TopDocs[] search= new TopDocs[offset];
		for (int i = 0; i < offset; i++) {
			search[i]= searcher.search(query, null, resperpage);
		}
		Date end = new Date();
		System.out.println("Time: " + (end.getTime() - start.getTime())
				+ "ms");

		//search[0].scoreDocs[0].
		//reader.close();
		} catch (Exception e){
			e.printStackTrace();
		}
